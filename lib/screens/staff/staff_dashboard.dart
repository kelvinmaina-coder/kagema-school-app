...
class _StaffDashboardState extends State<StaffDashboard> {
  ...
  Future<void> _loadDashboardData() async {
    ...
    if (user == null || user.email == null) {
       setState(() { _isLoading = false; _errorMessage = "Neural Identity missing. Please re-authorize session."; });
       return;
    }
    ...
    try {
      ...
      if (mounted) setState(() { _isLoading = false; _errorMessage = "Cloud Sync Error: Connection lost."; });
    }
  }
  ...
  Future<void> _handleCheckInOut() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Intelligence Logged: $newStatus'), backgroundColor: _isCheckedIn ? Colors.orange.shade800 : Colors.green.shade800, behavior: SnackBarBehavior.floating));
        _loadDashboardData();
      }
    } catch (e) {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [_buildSectionHeader('ACTIVE SYSTEM TASKS', Icons.assignment_turned_in_rounded), TextButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => TaskListScreen(staffId: _staffProfile?['staff_id'] ?? ""))), child: const Text('View All', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold)))]),
                  _buildTasksSection(theme, gemini),
                  const SizedBox(height: 40),
                  _buildSectionHeader('BROADCAST ANNOUNCEMENTS', Icons.campaign_rounded),
  ...
  Widget _buildRecordsTab(ThemeData theme, GeminiThemeExtension? gemini) {
    return CustomScrollView(
      slivers: [
        _buildHeroAppBar(theme, gemini, title: 'MY NEURAL HISTORY'),
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _buildSectionHeader('PAYSLIP NEXUS', Icons.account_balance_wallet_rounded),
                _buildPayslipCard(theme, gemini),
                const SizedBox(height: 48),
                _buildSectionHeader('LEAVE STATUS NEXUS', Icons.history_edu_rounded),
                _buildLeaveHistory(theme, gemini),
                const SizedBox(height: 48),
                _buildSectionHeader('ATTENDANCE LOG MATRIX', Icons.verified_user_rounded),
  ...
  Widget _buildIDCard(ThemeData theme, GeminiThemeExtension? gemini) {
    final content = Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(_staffProfile?['name']?.toString().toUpperCase() ?? 'AUTHORIZED STAFF', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18, letterSpacing: 1)),
        const SizedBox(height: 6),
        Text(_staffProfile?['role']?.toString().toUpperCase() ?? 'SYSTEM NODE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, color: theme.primaryColor, letterSpacing: 2)),
        const SizedBox(height: 8),
        Text('ID: ${_staffProfile?['staff_id'] ?? 'VERIFYING...'}', style: TextStyle(fontSize: 11, color: Colors.grey.shade500, fontWeight: FontWeight.bold)),
      ])),
  ...
  Widget _buildAttendancePanel(ThemeData theme, GeminiThemeExtension? gemini) {
    ...
    final content = Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(_isCheckedIn ? 'NODE: ACTIVE' : 'NODE: OFFLINE', style: TextStyle(fontWeight: FontWeight.w900, color: statusColor, letterSpacing: 1.5, fontSize: 12)), if (_isCheckedIn && _lastCheckInTime != null) Text('Duty start: $_lastCheckInTime', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.grey))])),
      ElevatedButton(onPressed: _handleCheckInOut, style: ElevatedButton.styleFrom(backgroundColor: statusColor, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)), elevation: 8), child: Text(_isCheckedIn ? 'TERMINATE' : 'INITIALIZE', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 10, letterSpacing: 1)))
    ]);
  ...
  Widget _buildHeroAppBar(ThemeData theme, GeminiThemeExtension? gemini, {String title = 'STAFF HUB'}) {
  ...
  Widget _buildAttendanceHistory(ThemeData theme, GeminiThemeExtension? gemini) {
    if (_attendanceHistory.isEmpty) return _emptyState(Icons.verified_user_rounded, 'NO ATTENDANCE LOGS');
  ...
  Widget _buildStaffDrawer(ThemeData theme) { return Drawer(backgroundColor: theme.scaffoldBackgroundColor, child: ListView(padding: EdgeInsets.zero, children: [DrawerHeader(decoration: BoxDecoration(gradient: LinearGradient(colors: [theme.primaryColor, Colors.indigo.shade900])), child: const Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.end, children: [Text('Kagema System', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900)), Text('STAFF REGISTRY ACCESS', style: TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1))])), ListTile(leading: const Icon(Icons.settings_rounded), title: const Text('Control Center', style: TextStyle(fontWeight: FontWeight.bold)), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SettingsScreen(role: 'Staff')))), ListTile(leading: const Icon(Icons.logout_rounded, color: Colors.red), title: const Text('End Session', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)), onTap: () => Navigator.pushReplacementNamed(context, '/login'))])); }
}
